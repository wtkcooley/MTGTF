...
var uid = "0ikfP5GuznSpUbqh2H0M9SMYWnm1"; //temporary
...
function returnCards(){
  ...
  function cardTableRow(name){
    ...
    tradeNowButton.addEventListener("click", function(event) {
      tempTradeBinder.push(name);
      console.log(tempTradeBinder);
      });
...
/* uploads the temp tradebinder cards to the online tradebinder */
function updateTradeBinder() {
  console.log("we made it");
  for(var i = 0; i < tempTradeBinder.length; i++){

    firebase.database().ref('users/' + uid).push({card: tempTradeBinder[i]})
    console.log(tempTradeBinder[i]);
  }
}
